                ...
                if (bytesRead === 24) {
                    ...
                    if (type === 1) { // EV_KEY
                        if (code === 42) { // Left Shift
                            ...
                            console.log(`[${devicePath}] Shift ${value === 1 ? 'pressed' : 'released'}`);
                        } else if (code === 58) { // Caps Lock
                            if (value === 1) {
                                this.isCapsLock = !this.isCapsLock;
                                console.log(`[${devicePath}] Caps Lock ${this.isCapsLock ? 'ON' : 'OFF'}`);
                            }
                        } else if (value === 1) { // Key press
                            const char = getCharFromKeyCode(code, this.isShiftPressed, this.isCapsLock);
                            if (char === '\n') {
                                console.log(`[${devicePath}] Enter pressed - Current line: "${this.currentLine}"`);
                                ...
                                console.log(`[${devicePath}] Backspace pressed - Current line: "${this.currentLine}"`);
                            } else if (char) {
                                this.currentLine += char;
                                console.log(`[${devicePath}] Key pressed: "${char}" - Current line: "${this.currentLine}"`);
                            }
                        }
                    }
                }
